...
MIT 授予的是版权许可，**不包含商标授权**。因此本项目使用**自有标记**
（`desktop/assets/app-mark.svg`，一个终端提示符）：不使用 DeepSeek 的标识，也不以其名称或
标识暗示官方背书。早期版本曾把内置运行时的 DeepSeek 鲸鱼 favicon 逐字节复制成应用图标，
**该素材已移除并替换**。

应用窗口里显示的是**真实运行的 Harness 前端**——那是上游产品本身，其中出现的任何上游品牌
属于上游，与本项目的标识无关。
