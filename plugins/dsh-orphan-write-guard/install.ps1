# Install the "orphan-write-guard" DSH plugin into a profile.
#
...
#
# This is a host-only plugin: it declares no `dsh.client`, so there is no
# browser bundle to copy and nothing to refresh.
...
$packageName = 'dsh-orphan-write-guard'
$rowId = 'orphan-write-guard'
...
# Read as UTF-8 explicitly: the patch layer carries non-ASCII comments and a
...
Write-Host "[3/3] done"
Write-Host ''
Write-Host "Host-only plugin: the 'web' profile reloads user patches live, so the guard"
Write-Host "is active as soon as the patch layer is rewritten. No browser refresh and no"
Write-Host "desktop restart are needed."
