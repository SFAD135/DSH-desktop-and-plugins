# Remove the "orphan-write-guard" DSH plugin from a profile.
#
#   powershell -ExecutionPolicy Bypass -File uninstall.ps1
#
# Deletes <profile>/node_modules/dsh-orphan-write-guard and reverts the Loader
# row in <profile>/cordis.patch.yml (from the install-time backup when one
# exists, otherwise by removing the inserted lines).
#
# No settings are written by this plugin, so there is nothing else to clean up.
