...
const packageName = 'dsh-global-prompt'
const checks = []
const failures = []
...
/* 1. Node resolution from the profile anchor — the Loader's first anchor is the
      profile directory for a bare row name. */
...
const requirePkg = createRequire(join(profileDir, 'package.json'))
const pkgPath = requirePkg.resolve(`${packageName}/package.json`)
...
/* 3. The client bundle the module system would serve. */
...
check('client bundle carries no source-map sibling (optional)', true)

/* 4. The host half imports cleanly — including its schemastery dependency,
...
/* 5. The Loader row is present in the profile patch layer. */
const patchPath = join(profileDir, 'cordis.patch.yml')
const patch = existsSync(patchPath) ? readFileSync(patchPath, 'utf8') : ''
check('cordis.patch.yml mentions the package', patch.includes(packageName))
