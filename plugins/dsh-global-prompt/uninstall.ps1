# Remove the "global-prompt" DSH plugin from a profile.
#
#   powershell -ExecutionPolicy Bypass -File uninstall.ps1
#
# Deletes <profile>/node_modules/dsh-global-prompt and reverts the Loader row in
# <profile>/cordis.patch.yml (from the install-time backup when one exists,
# otherwise by removing the inserted lines).
# The "global-prompt:" section in settings.yaml is left in place on purpose, so
# a reinstall keeps the text. Delete it by hand to drop the value entirely.
