# Install the "archived-sessions" DSH plugin into a profile.
#
...
# so a development `node_modules` link (used to run verify.mjs offline) stays
...
Write-Host "[3/3] done"
Write-Host ''
Write-Host "Next: the 'web' profile reloads user patches live, so the host half normally"
Write-Host "hot-mounts. Refresh the Web GUI, then open Settings -- a page labelled"
Write-Host "'已归档会话' appears next to the shipped ones. Restart the desktop app if it"
Write-Host "does not show up."
