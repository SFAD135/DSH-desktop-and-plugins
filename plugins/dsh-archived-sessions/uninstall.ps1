# Remove the "archived-sessions" DSH plugin from a profile.
#
#   powershell -ExecutionPolicy Bypass -File uninstall.ps1
#
# Deletes <profile>/node_modules/dsh-archived-sessions and reverts the Loader row
# in <profile>/cordis.patch.yml (from the install-time backup when one exists,
# otherwise by removing the inserted lines).
#
# Sessions already restored through the page stay restored -- this only removes
# the surface. The "archived-sessions:" section in settings.yaml is left in
# place on purpose, so a reinstall keeps nothing surprising; its request/result
# entries are transient and harmless. Delete it by hand to drop them.
...
$packageName = 'dsh-archived-sessions'
$rowId = 'archived-sessions'
